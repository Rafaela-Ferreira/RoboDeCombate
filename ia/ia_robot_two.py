...
# ===== CONFIGURAÇÕES =====
USE_ESP32 = False   # >>> coloque True para rodar com ESP, False para simulação
ESP32_IP = "http://192.168.4.1"
STREAM_URL = f"{ESP32_IP}:81/stream"

# Coloque aqui um vídeo de teste ou use 0 para webcam
VIDEO_TEST = 0 # "teste_incendio.mp4"
...
def capturar_stream():
    ...
    if USE_ESP32:
        ...
        cap = cv2.VideoCapture(VIDEO_TEST)  # vídeo de teste
...
def enviar_comando(endpoint):
    if USE_ESP32:
        ...
        print(f"[SIMULAÇÃO CMD] {endpoint}")
...
try:
    while True:
        ...
        cv2.imshow("RoboFIRE Visão IA (SIMULAÇÃO)", frame)
